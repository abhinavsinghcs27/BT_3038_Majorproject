...
// ─── Utility helpers (ported from frontend mockAi.js) ──────────────────────
...
function getProfileCompletion(profile) {
  ...
  const optionalBoost =
    (profile.report ? 1 : 0) +
    (Array.isArray(profile.conditions) && profile.conditions.length > 0 ? 1 : 0) +
    (Array.isArray(profile.symptoms) && profile.symptoms.length > 0 ? 1 : 0) +
  ...
  return { percentage, missingFields, isReady: missingFields.length === 0 };
}
...
exports.runAnalysis = (req, res) => {
  ...
  if (!profile || Object.keys(profile).length === 0) {
    return res.status(400).json({ error: 'Health profile data is missing' });
  }

  const userId   = req.headers['x-user-id'] || 'default';
  const userName = req.headers['x-user-name'] || 'Patient';
  ...
  if (age >= 55) {
    score += 14;
    drivers.push(createDriver('Age profile', `${age} years`, 'Elevated', 'Age-related risk factors increase monitoring intensity.', 14));
  } else if (age >= 40) {
    score += 8;
    drivers.push(createDriver('Age profile', `${age} years`, 'Moderate', 'Mid-life risk factors suggest preventive follow-up.', 8));
  }

  // Blood pressure
  if (systolic >= 145 || diastolic >= 95) {
    score += 20;
    drivers.push(createDriver('Blood pressure', `${systolic}/${diastolic}`, 'High', 'Blood pressure is above the desired target and strongly increases the score.', 20));
  } else if (systolic >= 130 || diastolic >= 85) {
    score += 10;
    drivers.push(createDriver('Blood pressure', `${systolic}/${diastolic}`, 'Moderate', 'Borderline elevated blood pressure raises cardiometabolic risk.', 10));
  } else if (systolic > 0 && diastolic > 0) {
    score -= 4;
    drivers.push(createDriver('Blood pressure', `${systolic}/${diastolic}`, 'Protective', 'Blood pressure is within a healthier operating range.', -4));
  }

  // Glucose
  if (glucose >= 140) {
    score += 16;
    drivers.push(createDriver('Glucose', `${glucose} mg/dL`, 'High', 'Elevated glucose is a major signal for metabolic stress.', 16));
  } else if (glucose >= 110) {
    score += 8;
    drivers.push(createDriver('Glucose', `${glucose} mg/dL`, 'Moderate', 'Glucose levels are above optimal and worth monitoring.', 8));
  }

  // BMI
  if (bmi >= 30) {
    score += 12;
    drivers.push(createDriver('Body mass index', bmi.toFixed(1), 'High', 'BMI indicates added cardiometabolic burden.', 12));
  } else if (bmi >= 25) {
    score += 6;
    drivers.push(createDriver('Body mass index', bmi.toFixed(1), 'Moderate', 'BMI suggests opportunity for weight optimization.', 6));
  } else if (bmi > 0) {
    score -= 3;
  }

  // Heart rate
  if (heartRate >= 100 || (heartRate > 0 && heartRate <= 52)) {
    score += 8;
    drivers.push(createDriver('Heart rate', `${heartRate} bpm`, 'Moderate', 'Heart rate is outside the common resting range.', 8));
  }

  // Oxygen
  if (oxygen > 0 && oxygen <= 94) {
    score += 10;
    drivers.push(createDriver('Oxygen saturation', `${oxygen}%`, 'High', 'Lower oxygen saturation can indicate respiratory strain.', 10));
  }

  // Sleep
  if (sleepHours > 0 && sleepHours < 6) {
    score += 8;
    drivers.push(createDriver('Sleep quality', `${sleepHours} hrs`, 'Moderate', 'Reduced sleep can amplify inflammation and recovery issues.', 8));
  ...
  if (profile.activityLevel === 'sedentary') {
    score += 8;
    drivers.push(createDriver('Activity level', 'Sedentary', 'Moderate', 'Low movement reduces resilience and long-term health capacity.', 8));
  ...
  // Conditions & symptoms
  score += Math.min(conditions.length * 5, 18);
  score += Math.min(symptoms.length * 3, 15);

  if (conditions.length > 0) {
    drivers.push(createDriver(
      'Known conditions',
      conditions.join(', '),
      conditions.length >= 3 ? 'High' : 'Moderate',
      'Pre-existing conditions increase baseline risk and care complexity.',
      Math.min(conditions.length * 5, 18),
    ));
  }

  if (profile.report) score -= 2;

  // Final scores
  const riskScore  = clamp(Math.round(score), 12, 96);
  const confidence = clamp(Math.round(68 + completion.percentage * 0.24 + (profile.report ? 5 : 0)), 70, 97);
  ...
  // Top 4 drivers sorted by impact
  ...
  const prediction = {
    ...
    carePriority,
    reviewWindow,
    headline: `${firstName}'s ${riskLevel.toLowerCase()}-risk care outlook`,
    summary:
      riskLevel === 'Low'
  ...
  // Return without internal userId field
  const { userId: _uid, ...response } = prediction;
